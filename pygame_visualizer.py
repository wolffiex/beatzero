...
class PulseCircle:
    def __init__(self, color, max_radius, x=WIDTH//2, y=HEIGHT//2, decay_rate=0.05, pulsate=True):
        self.x = x
        self.y = y
        self.color = color
    ...
    def draw(self, surface):
        if self.active or self.pulsate:
            ...
            circle_surface = pygame.Surface((draw_radius * 2, draw_radius * 2), pygame.SRCALPHA)
            pygame.draw.circle(circle_surface, color_with_alpha, (draw_radius, draw_radius), draw_radius)
...
def main():
    ...
    pulse_circles = {
        ...
        "bpm": PulseCircle(COLORS["bpm"], 450, WIDTH//2, HEIGHT//2, decay_rate=0.02, pulsate=True),
    }
    
    frequency_bands = FrequencyBands()
    
    # Particle systems for kick and hihat
    kick_particles = ParticleSystem(COLORS["kick"], WIDTH//2, HEIGHT//2, lifetime=0.8)
    hihat_particles = ParticleSystem(COLORS["hihat"], WIDTH//2, HEIGHT//2, lifetime=0.5)
        ...
        if latest_data:
            ...
            for method in pulse_circles:
                if method in ["energy", "hfc", "complex", "phase", "specflux"]:
                    if method in latest_data["onsets"] and latest_data["onsets"][method]["is_beat"]:
