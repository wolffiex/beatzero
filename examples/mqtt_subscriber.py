...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        console.print(
            f"[bold red]Failed to connect to MQTT broker with code: {rc}[/bold red]"
        )
...
def create_display_table(data):
    ...
    for method, method_desc in METHOD_DESCRIPTIONS.items():
        if method in data["onsets"]:
            ...
            beat_display = (
                "[bold green]YES[/bold green]" if is_beat else "[dim]no[/dim]"
            )
...
def main():
    ...
    console.print(
        "[bold]Waiting for music detection data... Press Ctrl+C to stop[/bold]"
    )
