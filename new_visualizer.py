...
class OnsetDetectionVisualizer:
    ...
    def draw(self, surface):
        ...
        for i, method in enumerate(self.onset_methods):
            ...
            base_color = COLORS.get(method, (200, 200, 200))  # Default to light gray if not found
            
            # Calculate brightness based on activity level
            activity = self.active_levels[method]
            min_brightness = 0.2
            scaled_r = int(base_color[0] * (min_brightness + activity * (1 - min_brightness)))
            scaled_g = int(base_color[1] * (min_brightness + activity * (1 - min_brightness)))
            scaled_b = int(base_color[2] * (min_brightness + activity * (1 - min_brightness)))
